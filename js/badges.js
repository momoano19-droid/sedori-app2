...
let storeBasicEditResolver = null;
let storeBasicEditTargetIndex = -1;

function ensureStoreBasicEditModal() {
  if (document.getElementById("storeBasicEditModal")) return;

  const modal = document.createElement("div");
  modal.id = "storeBasicEditModal";
  modal.className = "qtyCategoryModal";
  modal.innerHTML = `
    <div class="qtyCategoryCard">
      <div class="qtyCategoryTitle">店舗基本情報を編集</div>
      <div class="qtyCategorySub">最終訪問日はカレンダーから選べます</div>

      <div class="qtyCategorySectionTitle">店舗名</div>
      <div class="qtyManualRow">
        <input id="storeBasicEditName" class="qtyManualInput" type="text" placeholder="店舗名">
      </div>

      <div class="qtyCategorySectionTitle">都道府県</div>
      <div class="qtyManualRow">
        <input id="storeBasicEditPref" class="qtyManualInput" type="text" placeholder="都道府県">
      </div>

      <div class="qtyCategorySectionTitle">住所</div>
      <div class="qtyManualRow">
        <input id="storeBasicEditAddress" class="qtyManualInput" type="text" placeholder="住所">
      </div>

      <div class="qtyCategorySectionTitle">GoogleマップURL</div>
      <div class="qtyManualRow">
        <input id="storeBasicEditMapUrl" class="qtyManualInput" type="text" placeholder="GoogleマップURL">
      </div>

      <div class="qtyCategorySectionTitle">最終訪問日</div>
      <div class="qtyManualRow">
        <input id="storeBasicEditLastVisitDate" class="qtyManualInput" type="date">
      </div>

      <div class="row2 mt8">
        <button type="button" class="ghostBtn" onclick="clearStoreBasicEditDate()">最終訪問日をクリア</button>
      </div>

      <div class="categoryPickerActions">
        <button type="button" class="ghostBtn" onclick="closeStoreBasicEditModal(null)">キャンセル</button>
        <button type="button" class="primaryBtn" onclick="saveStoreBasicEditModal()">保存</button>
      </div>
    </div>
  `;

  modal.addEventListener("click", e => {
    if (e.target === modal) closeStoreBasicEditModal(null);
  });

  document.body.appendChild(modal);
}

function openStoreBasicEditModal(index) {
  const s = stores[index];
  if (!s) return Promise.resolve(null);

  ensureStoreBasicEditModal();
  storeBasicEditTargetIndex = index;

  const modal = document.getElementById("storeBasicEditModal");
  const nameEl = document.getElementById("storeBasicEditName");
  const prefEl = document.getElementById("storeBasicEditPref");
  const addressEl = document.getElementById("storeBasicEditAddress");
  const mapUrlEl = document.getElementById("storeBasicEditMapUrl");
  const lastVisitEl = document.getElementById("storeBasicEditLastVisitDate");

  if (nameEl) nameEl.value = s.name || "";
  if (prefEl) prefEl.value = s.pref || "";
  if (addressEl) addressEl.value = s.address || "";
  if (mapUrlEl) mapUrlEl.value = s.mapUrl || "";
  if (lastVisitEl) lastVisitEl.value = s.lastVisitDate || "";

  if (modal) modal.classList.add("show");

  return new Promise(resolve => {
    storeBasicEditResolver = resolve;
  });
}

function clearStoreBasicEditDate() {
  const lastVisitEl = document.getElementById("storeBasicEditLastVisitDate");
  if (lastVisitEl) lastVisitEl.value = "";
}

function closeStoreBasicEditModal(result) {
  const modal = document.getElementById("storeBasicEditModal");
  if (modal) modal.classList.remove("show");

  if (storeBasicEditResolver) {
    storeBasicEditResolver(result);
    storeBasicEditResolver = null;
  }

  storeBasicEditTargetIndex = -1;
}

function saveStoreBasicEditModal() {
  const nameEl = document.getElementById("storeBasicEditName");
  const prefEl = document.getElementById("storeBasicEditPref");
  const addressEl = document.getElementById("storeBasicEditAddress");
  const mapUrlEl = document.getElementById("storeBasicEditMapUrl");
  ...
  const mapUrl = String(mapUrlEl?.value || "").trim();
  const lastVisitDate = String(lastVisitEl?.value || "").trim();
  ...
  if (lastVisitDate && !/^\d{4}-\d{2}-\d{2}$/.test(lastVisitDate)) {
    alert("最終訪問日は YYYY-MM-DD 形式で入力してください。");
    return;
  }

  closeStoreBasicEditModal({
    name,
    pref,
    address,
    mapUrl,
...
番号を入力してください`,
  ...
  if (menu === "1") {
    const result = await openStoreBasicEditModal(i);
    if (!result) return;

    s.name = result.name;
    s.pref = result.pref;
    s.address = result.address;
    s.mapUrl = result.mapUrl;
    s.lastVisitDate = result.lastVisitDate || "";
