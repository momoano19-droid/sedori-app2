...
function renderBusinessInfoHtml(store, options = {}) {
  ...
  const showBusinessStatus =
    businessStatus.code !== "unknown" &&
    businessStatus.code !== "open";
...
function render() {
  ...
  const signature = JSON.stringify({
    ...
    storeCounters: stores.map(s =>
      `${s.id}:${Number(s.visits || 0)}:${Number(s.buyDays || 0)}:${Number(s.items || 0)}`
    ).join("|"),

    storeCategories: stores.map(s =>
      `${s.id}:${JSON.stringify(s.categoryCounts || {})}`
    ).join("|"),

    logsSignature: logs.map(l =>
      `${l.id || ""}:${l.storeId || ""}:${l.type || ""}:${l.delta || 0}:${l.date || ""}:${l.category || ""}`
    ).join("|"),
  ...
  modal.innerHTML = `
    <div class="qtyCategoryCard">
      ...
      <div class="storeHoursGrid">
        <input id="editOpenTime" type="text" inputmode="numeric" placeholder="開店時間（例 1000）">
        <input id="editCloseTime" type="text" inputmode="numeric" placeholder="閉店時間（例 2100）">
      </div>
...
function openStoreEditModal(index) {
  ...
  setupTimeInputAutoFormatForIds(["editOpenTime", "editCloseTime"]);
...
async function saveStoreEditModal() {
  ...
  if (openEl) openEl.value = formatTimeInputValue(openEl.value);
  if (closeEl) closeEl.value = formatTimeInputValue(closeEl.value);
...
function setupTimeInputAutoFormatForIds(ids = []) {
  ids.forEach(id => {
...
function setupTimeInputAutoFormat() {
  setupTimeInputAutoFormatForIds(["openTime", "closeTime"]);
}
