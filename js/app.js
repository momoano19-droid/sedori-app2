    ...
    <div class="item compactCard ${expectedClass} ${staleClass}">
      <div class="storeCardTopLine">
        ...
        <button
          ...
          aria-label="ããã¡ã¢ãå¥åã»ç·¨é"
          title="ã¿ãããã¦ããã¡ã¢ãå¥åã»ç·¨é"
        >
          <span class="storeMiniMemoScroll">${escapeHtml(getStoreMiniMemo(s.id) || "ããã¡ã¢")}</span>
        </button>
      </div>
      ...
      return `
        <div class="item todayRouteItem ${visited ? "todayRouteItemVisited" : ""}">
          ...
          <div class="todayRouteExpected">
            ð° æå¾å¤
            <strong>${Math.round(m.expected).toLocaleString()}å</strong>
          </div>
